...
def get_ticker_info_cached(ticker: str) -> dict:
    ...
    for attempt in range(3):
        try:
            ...
            # Try fast_info as fallback (different endpoint, sometimes more reliable)
    ...
    raise ValueError("Failed to fetch ticker info after retries")
...
def render_snapshot_page(ticker: str, df: pd.DataFrame, desc_map: dict):
    ...
    # Debug expander (remove after troubleshooting)
    with st.expander("🔧 Debug Info (remove after troubleshooting)", expanded=False):
        st.write("**Ticker Info Keys:**", list(info.keys()) if info else "EMPTY - No data returned")
        st.write("**Price Data:**", price_data)
        st.write("**Market Cap raw:**", info.get("marketCap"))
        st.write("**Volume raw:**", info.get("volume"))
        st.write("**Beta raw:**", info.get("beta"))
        if not info:
            st.error("❌ get_ticker_info() returned empty dict - API likely blocked")
        if not price_data.get("valid"):
            st.error("❌ get_ticker_price_data() failed - price API likely blocked")
