...
if st.session_state.view_mode_state == "Conviction Ranking":
    ...
    # ===== CONVICTION RANKING STYLES (scoped to this tab only) =====
    def inject_conviction_styles():
        st.markdown('''
        <style>
        /* Header */
        .conv-header {
    ...
    try:
        ...
        if not leaders_df.empty:
            ...
            date_labels_short = [d.strftime("%m/%Y") for d in unique_dates]
            ...
            if not top_data.empty:
                ...
                st.markdown('''
                    <div class="conv-header">
                        <div class="conv-title">Conviction Rankings</div>
                ...
                st.markdown(f'''
                    <div class="ticker-strip">
                        <div class="ticker-item">
                            <span class="ticker-label">Viewing</span>
                            <span class="ticker-value">{date_labels[selected_idx]}</span>
                        </div>
                        <span class="ticker-divider">|</span>
                        <div class="ticker-item">
                            <span class="ticker-label">#1 This Month</span>
                            <span class="ticker-value highlight">{current_month_leader}</span>
                        </div>
                        <span class="ticker-divider">|</span>
                        <div class="ticker-item">
                            <span class="ticker-label">All-Time Leader</span>
                            <span class="ticker-value">{current_leader}</span>
                            <span style="color:#64748b;font-size:0.75rem;">({current_count}mo)</span>
                        </div>
                        <span class="ticker-divider">|</span>
                        <div class="ticker-item">
                            <span class="ticker-label">Unique #1s</span>
                            <span class="ticker-value">{total_unique}</span>
                        </div>
                        <span class="ticker-divider">|</span>
                        <div class="ticker-item">
                            <span class="ticker-label">Months</span>
                            <span class="ticker-value">{total_periods}</span>
                        </div>
                    </div>
                ''', unsafe_allow_html=True)
                ...
                # ===== TIME SCRUBBER =====
                st.markdown(f'''
                    <div class="scrubber-label">Time Range: {date_labels_short[0]} — {date_labels_short[-1]}</div>
                ''', unsafe_allow_html=True)
                ...
                st.markdown(f'''
                    <div class="conv-chart-header">
                        <span class="conv-chart-title">Cumulative #1 Rankings</span>
                        <span class="conv-chart-date">as of {date_labels[selected_idx]}</span>
                    </div>
                ''', unsafe_allow_html=True)
                ...
                # Blue color gradient matching app theme
                def get_bar_color(rank, total):
                    opacity = 1.0 - (rank - 1) / max(total - 1, 1) * 0.5
                    return f"rgba(122, 162, 255, {opacity})"

                colors = [get_bar_color(r, len(top_data)) for r in top_data["rank"]]
                ...
                fig_race.update_layout(
                    height=480,
                    margin=dict(t=10, l=60, r=40, b=40),
                    paper_bgcolor="rgba(0,0,0,0)",
                    plot_bgcolor="rgba(0,0,0,0)",
...
**Metrics explained:**
- **#1 This Month** — The stock with the highest sentiment score that month
- **All-Time Leader** — Most total months at #1 up to selected date
- **Unique #1s** — Different stocks that have ever reached #1
- **Months** — Total monthly rebalances covered
                    ''')
