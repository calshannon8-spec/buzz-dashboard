...
def get_ticker_info_cached(ticker: str) -> dict:
    ...
    # Let yfinance handle session internally (requires curl_cffi)
    ticker_obj = yf.Ticker(ticker)

    # Try .info endpoint first (often blocked on cloud)
    ...
    # Try direct Yahoo Finance API (works better on cloud)
    try:
        session = requests.Session()
        session.get("https://finance.yahoo.com", timeout=5)  # Get cookies first
        ...
        url = f"https://query1.finance.yahoo.com/v10/finance/quoteSummary/{ticker}?modules=price,defaultKeyStatistics,summaryDetail"
        ...
        # Try v7 endpoint as fallback (sometimes works when v10 is blocked)
        url_v7 = f"https://query2.finance.yahoo.com/v7/finance/quote?symbols={ticker}"
        resp = session.get(url_v7, headers=headers, timeout=10)
        if resp.status_code == 200:
            data = resp.json()
    ...
    # Final fallback: extract data from history() which works on cloud
    ...
    if not hist.empty and "Close" in hist.columns:
        ...
        fig.update_layout(
            ...
            xaxis=dict(showgrid=False, showline=False, zeroline=False,
                       tickfont=dict(color="#6b7a8a", size=10), fixedrange=True),
            yaxis=dict(showgrid=True, gridcolor="rgba(255,255,255,0.04)",
                       showline=False, zeroline=False,
                       tickfont=dict(color="#6b7a8a", size=10),
                       tickprefix="" if compare_sp500 else "$", fixedrange=True),
            hovermode="x unified",
            dragmode=False,
    ...
    with col_chart:
        ...
        if not hist.empty and "Close" in hist.columns:
            ...
            fig.update_layout(
                ...
                yaxis=dict(showgrid=True, gridcolor="rgba(255,255,255,0.04)",
                           showline=False, zeroline=False,
                           tickfont=dict(color="#6b7a8a", size=10), tickprefix="$", fixedrange=True),
