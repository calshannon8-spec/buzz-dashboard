...
def get_ticker_price_data_cached(ticker: str) -> dict:
    """Fetch ticker price. Raises exception on failure (won't be cached)."""
    ticker_obj = yf.Ticker(ticker)
...
@st.cache_data(ttl=600)  # Cache for 10 minutes
def get_ticker_news_cached(ticker: str) -> list:
    """Fetch ticker news. Raises exception on failure (won't be cached)."""
    t = yf.Ticker(ticker)
...
def get_ticker_news(ticker: str) -> list:
    """Wrapper that handles errors gracefully."""
    try:
        return get_ticker_news_cached(ticker)
    except Exception:
        return []
