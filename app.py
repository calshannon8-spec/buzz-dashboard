...
def get_ticker_info(ticker: str) -> dict:
    """Fetch ticker info with caching to avoid rate limits."""
    try:
...
def get_ticker_calendar(ticker: str) -> dict | None:
    """Fetch ticker calendar (earnings dates) with caching to avoid rate limits."""
    try:
        ticker_obj = yf.Ticker(ticker)
...
def get_ticker_price_data(ticker: str) -> dict:
    """Fetch ticker price and daily change with caching to avoid rate limits."""
    try:
        ticker_obj = yf.Ticker(ticker)
...
def get_ticker_news(ticker: str) -> list:
    """Fetch ticker news with caching to avoid rate limits."""
    try:
        t = yf.Ticker(ticker)
...
if st.session_state.view_mode_state == "Snapshot":
    ...
    # Fetch live price for snapshot view (using cached function to avoid rate limits)
    price_data = get_ticker_price_data(selected_ticker)
    if price_data["valid"]:
...
if st.session_state.view_mode_state == "BUZZ Performance":
    # Fetch BUZZ ETF info for total net assets (using cached function to avoid rate limits)
    ...
    # Get daily change (using cached function to avoid rate limits)
    ...
    if buzz_price_data["valid"]:
        current_price = buzz_price_data["price"]
        change_pct = buzz_price_data["pct_change"]
        # Estimate daily change from pct (we don't have prev_close directly but can calculate)
